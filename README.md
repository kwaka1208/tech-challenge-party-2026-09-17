...
- 星空部分のフルスクリーン表示
- モバイル全画面でGPSと端末コンパスに追従し、端末を向けた方向の天体を表示
...
方位センサーを備えたモバイル端末で星図をフルスクリーンにすると、GPSの観測地点と端末の向きに合わせた透視表示へ切り替わります。端末の背面を実際の空へ向けると、その方向にある恒星・惑星が画面中央へ表示されます。

- iOSなど権限確認が必要な端末では、最初に「センサー許可」をタップし、許可後にもう一度全画面ボタンをタップします。
- 全画面右上の「コンパス ON/OFF」で追従と通常の手動星図を切り替えられます。
